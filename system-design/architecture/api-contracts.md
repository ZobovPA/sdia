...
Read-модель может вернуть, в том числе, терминальные статусы `COMPLETED`, `FAILED` и `EXPIRED`. Статус `EXPIRED` означает, что hold был снят до старта внешнего шага, потому что `PaymentInitiated` не был опубликован вовремя.

### `GET /api/users/{userId}/payments`

Назначение: получить историю платежей пользователя из read-модели.

История пользователя возвращает те же terminal statuses, что и чтение по `paymentId`, включая `EXPIRED`.
