...
Центральной точкой оркестрации является `Payments Context`: он не хранит баланс и не ведёт бухгалтерский учёт, а управляет жизненным циклом платежа, меняет его статусы и публикует доменные события. `Wallet Context` отвечает за операционный баланс и `hold`-операции. `Anti-Fraud` принимает онлайн-решение до движения денег. `Core Integration / ACL` изолирует legacy core. `Callbacks` и `Notifications` получают события о статусах платежей и не блокируют основной сценарий.
...
| `Anti-Fraud / Scoring` | онлайн-оценка риска и причины решения | `RiskAssessmentId`, `Decision`, `RiskScore` | получает синхронные запросы на решение и асинхронные сигналы для обучения |
...
- `Anti-Fraud` получает синхронный запрос на решение от `Payments`, а также асинхронные сигналы от `Payments`, `Wallet` и `Customer`.
...
- `Payments -> Anti-Fraud`: `Customer/Supplier`, синхронный вызов `AssessRisk`
- `Payments -> Callbacks` и `Payments -> Notifications`: `Published Language`, событие `PaymentStatusChanged`
...
- `Wallet -> Anti-Fraud` и `Payments -> Anti-Fraud`: асинхронные сигналы для моделей и правил риска

`Anti-Fraud` в этой схеме не является просто подписчиком событий. Это отдельный контекст с двумя видами связей:

- входные контексты для него: `Payments`, `Wallet`, `Customer`, а также внешние сигналы из каналов и провайдеров
- потребители его решений: `Payments`, `Notifications` и, при необходимости, `Case Management`
...
На схеме сервисы и их хранилища сгруппированы по bounded context-ам. Инфраструктурные компоненты, такие как `Kong`, `Kafka` и `RabbitMQ`, вынесены отдельно, потому что обслуживают несколько контекстов сразу.
...
8. `Notifications`, `Callbacks` и `Analytics` потребляют событие независимо друг от друга.
...
| `Wallet -> Anti-Fraud` | асинхронно | `Kafka` | аномалии баланса и паттерны по holds полезны для правил, но не должны блокировать операции | допустима отложенная согласованность |
...
- фрод-сигналы и feature events для последующего обучения
...
- `Analytics` обновляет витрины и транзакционные ленты
- `Anti-Fraud` использует событие как дополнительный сигнал для моделей
...
| `PaymentRequested -> Anti-Fraud` | нет + `Kafka` | синхронная команда + event | решение нужно получить синхронно по RPC, а сигналы и аудит удобно публиковать в журнал событий |
| Телеметрия и audit stream | `Kafka` | event | высокий throughput, долговременное хранение и повторный проигрыш событий |
...
`Kong` выбран потому, что для банковского пограничного слоя важны зрелые плагины безопасности, производительность на базе `Nginx`, поддержка `OIDC`, `mTLS`, `ACL`, rate-limiting и хорошая встраиваемость в Kubernetes и стек наблюдаемости. Альтернативы вроде `Spring Cloud Gateway` и `Traefik` возможны, но для высоконагруженного внешнего контура и богатой плагинной модели в этом кейсе `Kong` выглядит практичнее.
